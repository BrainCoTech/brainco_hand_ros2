#!/usr/bin/env python3
"""
Gazebo-MoveIt 整合 Launch 文件 - Revo2 双手
启动双 Revo2 灵巧手的 Gazebo 仿真 + MoveIt 运动规划系统

此 launch 文件整合了：
1. Gazebo 仿真环境（使用 GazeboSimSystem）
2. MoveIt 运动规划功能（支持双手独立规划）
3. RViz 可视化界面
...
def generate_launch_description():
    ...
    # 注意：左右手控制器并行启动，我们在右手控制器退出后启动 MoveGroup
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
