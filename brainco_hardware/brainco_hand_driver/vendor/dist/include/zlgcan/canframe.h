...
#define TX_DELAY_SEND_FLAG \
  0x80 /* indicat tx frame in delay send mode, 1:send in device queue; 0:send direct to bus */
#define IS_DELAY_SEND(flag) \
  (!!(                      \
    flag &                  \
    TX_DELAY_SEND_FLAG))  // 1:delay send frame with delay time in __res0/__res0 0:normal frame
#define TX_DELAY_SEND_TIME_UNIT_FLAG \
  0x40 /* indicat tx delay send time unit, 1:time unit is 100us; 0:time unit is 1ms */
#define IS_DELAY_SEND_TIME_UNIT_MS(flag) \
  (!(flag & TX_DELAY_SEND_TIME_UNIT_FLAG))  // 0: time unit 100us 1: time unit 1ms
#define IS_DELAY_SEND_TIME_UNIT_100US(flag) \
  (!!(flag & TX_DELAY_SEND_TIME_UNIT_FLAG))  // 1: time unit 100us 0: time unit 1ms

/* TX_ECHO_FLAG apply to can_frame.__pad and canfd_frame.flags, apply to tx and rx frames */
#define TX_ECHO_FLAG \
  0x20 /* indicat tx frame will be echoed(recieved) when tx frame trasmit to the bus*/
