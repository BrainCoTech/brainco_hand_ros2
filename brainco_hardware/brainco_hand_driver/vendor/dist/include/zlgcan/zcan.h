...
typedef struct _VCI_LIN_SUBSCIBE_CFG
{
  ...
  U8 chkSumMode;   // 校验方式，0-默认，启动时配置 1-经典校验 2-增强校验
...
typedef struct _ZCAN_UDS_REQUEST
{
  ...
  struct
  {
    ...
    U32
      enhanced_timeout;  // 收到消极响应错误码为0x78后的超时时间(ms)。因PC定时器误差，建议设置不小于200ms
  ...
  struct
  {
    ...
    U8 local_st_min;  // 本程序发送流控时用，连续帧之间的最小间隔, 0x00-0x7F(0ms~127ms),
    ...
    U8 remote_st_min;  // 发送多帧时用, is_ignore_ecu_st_min = 1 时有效, 0x00-0x7F(0ms~127ms),
                       // 0xF1-0xF9(100us~900us)
...
EXTERN_C U32 ZCAN_API
VCI_Receive(U32 Type, U32 Card, U32 Port, ZCAN_20_MSG * pData, U32 Count, U32 Time);
EXTERN_C U32 ZCAN_API
VCI_ReceiveFD(U32 Type, U32 Card, U32 Port, ZCAN_FD_MSG * pData, U32 Count, U32 Time);
...
EXTERN_C U32
VCI_ReceiveLIN(U32 Type, U32 Card, U32 LinChn, PZCAN_LIN_MSG pReceive, U32 Len, int WaitTime);
EXTERN_C U32 VCI_SetLINSubscribe(
  U32 Type, U32 Card, U32 LinChn, PZCAN_LIN_SUBSCIBE_CFG pSend, U32 nSubscribeCount);
EXTERN_C U32
VCI_SetLINPublish(U32 Type, U32 Card, U32 LinChn, PZCAN_LIN_PUBLISH_CFG pSend, U32 nPublishCount);

// 合并接收
EXTERN_C U32
VCI_TransmitData(unsigned Type, unsigned Card, unsigned Port, ZCANDataObj * pData, unsigned Count);
EXTERN_C U32 VCI_ReceiveData(
  unsigned Type, unsigned Card, unsigned Port, ZCANDataObj * pData, unsigned Count, unsigned Time);

// UDS
EXTERN_C U32 VCI_UDS_Request(
  unsigned Type, unsigned Card, const ZCAN_UDS_REQUEST * req, ZCAN_UDS_RESPONSE * resp,
  U8 * dataBuf, U32 dataBufSize);
EXTERN_C U32 VCI_UDS_Control(
  unsigned Type, unsigned Card, const ZCAN_UDS_CTRL_REQ * ctrl, ZCAN_UDS_CTRL_RESP * resp);
