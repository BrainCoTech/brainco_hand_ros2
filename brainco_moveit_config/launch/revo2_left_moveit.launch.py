"""
完整的 MoveIt demo launch 文件 - Revo2 Left Hand
启动 Revo2 左手的完整 MoveIt 演示环境（使用 FakeSystem）
...
def generate_launch_description():
    ...
    moveit_config = (
        MoveItConfigsBuilder("revo2_left", package_name="brainco_moveit_config")
        .robot_description(file_path="config/revo2_left.urdf.xacro")
        .robot_description_semantic(file_path="config/revo2_left.srdf")
        .trajectory_execution(file_path="config/revo2_left_moveit_controllers.yaml")
        .joint_limits(file_path="config/revo2_left_joint_limits.yaml")
        .robot_description_kinematics(file_path="config/revo2_left_kinematics.yaml")
        .to_moveit_configs()
    )
    ...
    ld.add_action(
        Node(
            ...
            parameters=[
                moveit_config.robot_description,
                str(moveit_config.package_path / "config/revo2_left_controllers.yaml"),
            ],
        )
    )
